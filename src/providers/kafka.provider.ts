...
export class KafkaFacade {
        ...
        .then(
          (metadata) => {
            for (const data of metadata) {
              Logger.debug(
                `Message successfully produced to topic [${data.topicName}(#${data.partition})].`
              );
  ...
  public static retry(stream: MessageStream): Promise<void> {
    if (stream.retries >= parseInt(getEnv("KAFKA_MAX_RETRIES", "10"))) {
  ...
  public static prepareStream(topic: string, data: unknown, retries = 0): MessageStream {
    return {
      body: data,
  ...
  public static async getConsumer(groupId: string): Promise<Consumer> {
    ...
    await consumer.connect().catch(logCatchedError);
  ...
  public static async getAdmin(): Promise<Admin> {
    ...
    await admin.connect().catch(logCatchedError);
    ...
    return new Promise((resolve, reject) => {
      ...
      if (getEnv("KAFKA_CREATE_TOPICS_ON_START", "false") == "true") {
        admin
          ...
          .then(
            async () => {
              const raw = getEnv("KAFKA_TOPICS", "default_topic").replace(" ", "").split(",");
              const topics: ITopicConfig[] = [];

              for (const topic of raw) {
                topics.push({
                  topic: topic,
                  numPartitions: getEnv("KAFKA_NUM_PARTITIONS")
                    ? parseInt(getEnv("KAFKA_NUM_PARTITIONS"))
                    : undefined,
                  replicationFactor: getEnv("KAFKA_REPLICATION_FACTOR")
                    ? parseInt(getEnv("KAFKA_REPLICATION_FACTOR"))
                    : undefined,
                });
              }
              ...
              await admin
                ...
                .then(
                  (result) => {
                    if (result) {
                      Logger.debug(
                        Lang.__("Topics [{{topics}}] successfully created.", {
                          topics: raw.join(", "),
                        })
                      );
                    } else {
                      Logger.debug(
                        Lang.__(`Topics [{{topics}}] were not created.`, {
                          topics: raw.join(", "),
                        })
                      );
                    }
            ...
            (error) => {
              Logger.error(
                Lang.__("Kafka admin cannot connect to kafka broker(s) [{{brokers}}].", {
                  brokers: brokers,
                })
              );
              logCatchedException(error);
      ...
      producer
        ...
        .then(
          async () => {
            ...
            Logger.debug(
              Lang.__(
                "Kafka producer successfully connected to kafka broker(s) on [{{brokers}}].",
                {
                  brokers: brokers,
                }
              )
            ...
            if (this.boostrap.consumers.length > 0) {
              for (const consumerClass of this.boostrap.consumers) {
                ...
                Logger.audit(
                  Lang.__("Preparing consumer [{{name}}({{group}})] on topic [{{topic}}].", {
                ...
                const consumer = (await KafkaFacade.getConsumer(instance.groupId).catch(
                  logCatchedError
                )) as Consumer;
                ...
                await instance.boot(consumer).then(() => {
                  consumer.run({
                    ...
                    eachMessage: async (payload) => {
                      ...
                      Logger.debug(
                        Lang.__(
                          "Consuming message on [{{name}}({{group}})] from topic [{{topic}}(#{{partition}})]",
                          {
                            name: instance.constructor.name,
                            group: instance.groupId,
                            topic: instance.topic,
                      ...
                      return instance
                        ...
                        .then(
                          () => {
                            Logger.debug(
                              Lang.__(
                                "Message successfully consumed on [{{name}}({{group}})] from topic [{{topic}}(#{{partition}})]",
                                {
                                  name: instance.constructor.name,
                                  group: instance.groupId,
                                  topic: instance.topic,
                  ...
                  Logger.audit(
                    Lang.__("Consumer [{{name}}({{group}})] on topic [{{topic}}] is ready.", {
                      name: instance.constructor.name,
                      group: instance.groupId,
                      topic: instance.topic,
                    })
              ...
              Logger.audit(
                Lang.__("Consumers set up completed [{{count}}].", {
